...
void myAlert(string type, string message)
  {
   ...
   if(type == "print")
     ...
     {
      Print(type+" | Buy Engulfing MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
   else if(type == "order")
     {
     }
     ...
     {
      Print(type+" | Buy Engulfing MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      if(Audible_Alerts) Alert(type+" | Buy Engulfing MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      handle = FileOpen("Buy Engulfing MT4.txt", FILE_TXT|FILE_READ|FILE_WRITE|FILE_SHARE_READ|FILE_SHARE_WRITE, ';');
      if(handle != INVALID_HANDLE)
        {
         FileSeek(handle, 0, SEEK_END);
         FileWrite(handle, type+" | Buy Engulfing MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
         FileClose(handle);
        }
      if(Push_Notifications) SendNotification(type+" | Buy Engulfing MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
  }
   ...
   for(int i = limit-1; i >= 0; i--)
     {
      ...
      if(   
      ...
      )
        {
         ...
         if(i == 0 && Time[0] != time_alert) { myAlert("indicator", "Buy Engulfing"); time_alert = Time[0]; } //Instant alert, only once per bar
        }
      else
        {
