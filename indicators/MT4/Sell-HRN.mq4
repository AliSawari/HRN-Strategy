...
#property indicator_label1 "Sell HRN"
...
void myAlert(string type, string message)
  {
   ...
   if(type == "print")
     ...
     {
      Print(type+" | Sell HRN MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
   else if(type == "order")
     {
     }
     ...
     {
      Print(type+" | Sell HRN MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      if(Audible_Alerts) Alert(type+" | Sell HRN MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      handle = FileOpen("Sell HRN MT4.txt", FILE_TXT|FILE_READ|FILE_WRITE|FILE_SHARE_READ|FILE_SHARE_WRITE, ';');
      if(handle != INVALID_HANDLE)
        {
         FileSeek(handle, 0, SEEK_END);
         FileWrite(handle, type+" | Sell HRN MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
         FileClose(handle);
        }
      if(Push_Notifications) SendNotification(type+" | Sell HRN MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
  }
   ...
   for(int i = limit-1; i >= 0; i--)
     {
      ...
      //Indicator Buffer 1
      if(
        // MA comparison in all 3 candles      
         ...
         && iMA(NULL, PERIOD_CURRENT, 8, 0, MODE_SMA, PRICE_CLOSE, i+3) < iMA(NULL, PERIOD_CURRENT, 20, 0, MODE_SMA, PRICE_CLOSE, i+3)
         && iMA(NULL, PERIOD_CURRENT, 20, 0, MODE_SMA, PRICE_CLOSE, i+3) < iMA(NULL, PERIOD_CURRENT, 50, 0, MODE_SMA, PRICE_CLOSE, i+3)
         // MA decreasing in all 3 candles
         ...
         && iMA(NULL, PERIOD_CURRENT, 8, 0, MODE_SMA, PRICE_CLOSE, i+2) < iMA(NULL, PERIOD_CURRENT, 8, 0, MODE_SMA, PRICE_CLOSE, i+3)
         ...
         // last bearish candle should close below MA 20
        ...
        {
         Buffer1[i] = High[1+i]; //Set indicator value at Candlestick High
         if(i == 0 && Time[0] != time_alert) { myAlert("indicator", "Sell HRN"); time_alert = Time[0]; } //Instant alert, only once per bar
        }
      else
        {
