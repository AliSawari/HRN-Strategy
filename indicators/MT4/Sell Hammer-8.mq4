...
#property indicator_label1 "Sell Hammer-8"
...
void myAlert(string type, string message)
  {
   ...
   if(type == "print")
     ...
     {
      Print(type+" | Sell Hammer-8 MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
   else if(type == "order")
     {
     }
     ...
     {
      Print(type+" | Sell Hammer-8 MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      if(Audible_Alerts) Alert(type+" | Sell Hammer-8 MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      handle = FileOpen("Sell Hammer-8 MT4.txt", FILE_TXT|FILE_READ|FILE_WRITE|FILE_SHARE_READ|FILE_SHARE_WRITE, ';');
      if(handle != INVALID_HANDLE)
        {
         FileSeek(handle, 0, SEEK_END);
         FileWrite(handle, type+" | Sell Hammer-8 MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
         FileClose(handle);
        }
      if(Push_Notifications) SendNotification(type+" | Sell Hammer-8 MT4 @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
  }
   ...
   for(int i = limit-1; i >= 0; i--)
     {
      ...
      if(   
        ...
        // last bullish candle's lowest body point should close below MA8
        && MathMin(Open[1+i], Close[1+i]) < iMA(NULL, PERIOD_CURRENT, 8, 0, MODE_SMA, PRICE_CLOSE, i+1)
        
        // Hammer's Upper Shadow should be at least 60% above MA8
        && High[1+i] - ( ( High[1+i] - MathMax(Open[1+i], Close[1+i]) ) * 0.6 ) >= iMA(NULL, PERIOD_CURRENT, 8, 0, MODE_SMA, PRICE_CLOSE, 1+i)
      ...
      )
        {
         ...
         if(i == 0 && Time[0] != time_alert) { myAlert("indicator", "Sell Hammer-8"); time_alert = Time[0]; } //Instant alert, only once per bar
        }
      else
        {
